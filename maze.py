  ...
  def as_svg(self,filename):
    ...
    for i in range(self.n_rows):
      for j in range(self.n_cols):
        ...
        if i==self.n_rows-1 or self.maze[i][j][BOTTOMWALL]:
            dwg.add(dwg.line(((i+1)*pas, j*pas), ((i+1)*pas, (j+1)*pas), stroke=svgwrite.rgb(10, 10, 16, '%')))
        if j==0:
            dwg.add(dwg.line((i*pas, 0), ((i+1)*pas, 0), stroke=svgwrite.rgb(10, 10, 16, '%')))
        if j==self.n_cols-1 or self.maze[i][j][RIGHTWALL]:
            dwg.add(dwg.line((i*pas, (j+1)*pas), ((i+1)*pas, (j+1)*pas), stroke=svgwrite.rgb(10, 10, 16, '%')))
  ...
  try:
    if n_rows > 0:
      ...
      for i in range(1,100):
        ...
        print(maze.as_svg("maze"+"{:03d}".format(i)))
  except MemoryError:
